...
int main() {
   ...
   // cout << Matrix[0][sizeOfMatrix];
   // cout << (log((delta * abs(1 - 1.8)) / 2.1)) / (log(Matrix[0][3]));
    double N = abs(log((delta * abs(1 - Matrix[0][sizeOfMatrix])) / Matrix[sizeOfMatrix - 1][sizeOfMatrix])) / (log(Matrix[0][sizeOfMatrix])) + 1;
    cout << N;
    for (int numberOfStep = 0; numberOfStep<N; numberOfStep++) {
        ...
        for (int i = 0; i < sizeOfMatrix; i++) {
            ...
            for (int j = 0; j < sizeOfMatrix; j++) {
                ...
                sumOfString = sumOfString + Matrix[i][j]*cloneB[j];
            }
       ...
       // cout << deltaCheck << "F"<<sizeOfMatrix<<"g";
    }
